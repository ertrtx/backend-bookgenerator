## Based on the work of @cene555 https://github.com/cene555/Imagen-pytorch
## Image generator from prompts using an implementation of Imagen
...
######### COMMENT OUT:
# prompt = ""
# promptsList = ['purple unicorn', 'red fish']
...
def generateImages(prompt, myNumber, currentDir,  project_id, bucket_name):
    ...
    imagen_pt = os.path.join(currentDir, 'esrgan/ImagenT5-3B/model.pt')
    model.load_state_dict(_fix_path(imagen_pt))
    ...
    upsampler = RealESRGANer(
        scale=netscale,
        model_path= os.path.join(currentDir, 'esrgan/Real-ESRGAN/experiments/pretrained_models/RealESRGAN_x4plus.pth'),
...
# for count, prompt in enumerate(promptsList):
#     print(count, prompt)
#     main(prompt, count)
